    ...
    with torch.no_grad():
        for batch_number, (sino, gt) in tqdm(enumerate(data)):
            ...
            if batch_number % 10 == 0:
                ...
                axes[1].axis('off')
                output_path = f"/h/245/yukthiw/admm_denomamba_red/img_{batch_number}_comparison_{timestamp}.png"
                plt.tight_layout()
                plt.savefig(output_path, dpi=300, bbox_inches='tight')
                plt.close(fig)
                plt.imsave(f"/h/245/yukthiw/admm_denomamba_red/img_{batch_number}_reconstruction_{timestamp}.png", reconstruction, cmap='gray')
    print(np.mean(psnrs))
    print(np.mean(ssims))
    print(np.mean(rmses))
    np.save(f"/h/245/yukthiw/admm_denomamba_red/psnrs_{timestamp}.npy",np.array(psnrs))
    np.save(f"/h/245/yukthiw/admm_denomamba_red/ssims_{timestamp}.npy",np.array(ssims))
    np.save(f"/h/245/yukthiw/admm_denomamba_red/rmses_{timestamp}.npy",np.array(rmses))
